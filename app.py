    ...
    for filename in os.listdir(KNOWN_FOLDER):
        ...
        try:
            result = DeepFace.verify(
                ...
                model_name="Dlib",  # ✅ Use Dlib model
                enforce_detection=False  # ✅ Allow partial/poor-quality faces
            )
            print(f"Compared with {filename}: Verified={result['verified']} | Distance={result['distance']}")
            if result["verified"]:
                return filename.split('.')[0]
        except Exception as e:
            print(f"Error comparing with {filename}: {e}")
...
st.title("📸 ESP32-CAM Face Recognition with DeepFace (Dlib)")
